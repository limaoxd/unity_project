...
public class AI_Falcius_Spear : AI
{
    ...
    void choose_atk(int act_num)
    {
        ...
        if (stamina <= 70 || player_dis > 4.5f) return; //return to idle
    ...
    void Movement()
    {
        ...
        switch (act_num)
        {
            ...
            case 4: //Atk2  triple slash
                obstacle.enabled = true;
                agent.enabled = false;
                if (timer < 0.34) movement = Vector3.zero;
                ...
                else if (timer < 0.55) {transform.rotation = Quaternion.Euler(0f, angle, 0f); movement = Quaternion.Euler(0f, targetAngle, 0f) * Vector3.forward.normalized * 2f * run_sp * Time.deltaTime; }
                else movement = Vector3.zero;

                if (timer >= 0.28 && timer <= 0.34) atkTrigger.GetComponent<atk_trigger>().atk = true;
                else if (timer >= 0.36 && timer <= 0.4) atkTrigger.GetComponent<atk_trigger>().atk = true;
                else if (timer >= 0.44 && timer <= 0.55) atkTrigger.GetComponent<atk_trigger>().atk = true;
                else atkTrigger.GetComponent<atk_trigger>().atk =  false;
            ...
            case 5:
                ...
                obstacle.enabled = true;
                agent.enabled = false;
                if (timer < 0.18) {transform.rotation = Quaternion.Euler(0f, angle, 0f); movement = Quaternion.Euler(0f, targetAngle, 0f) * Vector3.back.normalized * sp * 0.3f * Time.deltaTime; }
                else if(timer < 0.34) movement = Vector3.zero;
                else if (timer < 0.44) {transform.rotation = Quaternion.Euler(0f, angle, 0f); movement = Quaternion.Euler(0f, targetAngle, 0f) * Vector3.forward.normalized * 2f * run_sp * Time.deltaTime; }
                else movement = Vector3.zero;

                if (timer >= 0.4 && timer <= 0.47) atkTrigger.GetComponent<atk_trigger>().atk = true;
                else atkTrigger.GetComponent<atk_trigger>().atk = false;
